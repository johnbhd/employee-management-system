    ...
    return (
      <div className="hr-reports-table-scroll">
        <table className="hr-reports-table">
          ...
          <thead><tr><th scope="col">Employee</th><th scope="col">Department</th><th scope="col">Date</th><th scope="col">Schedule</th><th scope="col">Time in</th><th scope="col">Late minutes</th><th scope="col">Source</th><th scope="col">Validation</th><th scope="col">HR Verification</th></tr></thead>
          <tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.employeeName}</strong><span className="hr-reports-cell-meta">{record.employeeId}</span></td><td>{record.department}</td><td>{formatDate(record.date)}</td><td>{record.schedule}</td><td>{record.timeIn}</td><td>{record.lateMinutes ? `${record.lateMinutes} min` : "—"}</td><td><SourceBadge source={record.source} /></td><td><StatusBadge tone={record.validationTone}>{record.validationStatus}</StatusBadge></td><td><StatusBadge tone={record.hrVerificationStatus === "Verified" ? "success" : record.hrVerificationStatus === "Needs Correction" ? "warning" : "info"}>{record.hrVerificationStatus ?? "Pending Review"}</StatusBadge></td></tr>)}{records.length === 0 ? <EmptyReportRow columns={9} /> : null}</tbody>
        </table>
      </div>
    );
  }
    ...
    return (
      <div className="hr-reports-table-scroll">
        <table className="hr-reports-table">
          ...
          <thead><tr><th scope="col">Employee</th><th scope="col">Department</th><th scope="col">Date</th><th scope="col">Schedule</th><th scope="col">Time out</th><th scope="col">Undertime minutes</th><th scope="col">Source</th><th scope="col">Validation</th><th scope="col">HR Verification</th></tr></thead>
    ...
    return (
      <div className="hr-reports-table-scroll">
        <table className="hr-reports-table">
          ...
          <thead><tr><th scope="col">Employee</th><th scope="col">Department</th><th scope="col">Date</th><th scope="col">Schedule</th><th scope="col">Time in</th><th scope="col">Source</th><th scope="col">Validation</th><th scope="col">Correction status</th><th scope="col">HR Verification</th></tr></thead>
          <tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.employeeName}</strong><span className="hr-reports-cell-meta">{record.employeeId}</span></td><td>{record.department}</td><td>{formatDate(record.date)}</td><td>{record.schedule}</td><td>{record.timeIn}</td><td><SourceBadge source={record.source} /></td><td><StatusBadge tone={record.validationTone}>{record.validationStatus}</StatusBadge></td><td>{record.correctionStatus ? <StatusBadge tone={correctionTone(record.correctionStatus)}>{record.correctionStatus}</StatusBadge> : <span className="hr-reports-missing-value">Not submitted</span>}</td><td><StatusBadge tone={record.hrVerificationStatus === "Verified" ? "success" : record.hrVerificationStatus === "Needs Correction" ? "warning" : "info"}>{record.hrVerificationStatus ?? "Pending Review"}</StatusBadge></td></tr>)}{records.length === 0 ? <EmptyReportRow columns={9} /> : null}</tbody>
        </table>
      </div>
    );
  }
  ...
  return (
    <div className="hr-reports-table-scroll">
      <table className="hr-reports-table">
        ...
        <thead><tr><th scope="col">Employee</th><th scope="col">Department</th><th scope="col">Schedule</th><th scope="col">Time in</th><th scope="col">Time out</th><th scope="col">Source</th><th scope="col">Status</th><th scope="col">Validation</th><th scope="col">HR Verification</th><th scope="col">Payroll Readiness</th></tr></thead>
        <tbody>{records.map((record) => <tr key={record.id}><td><strong>{record.employeeName}</strong><span className="hr-reports-cell-meta">{record.employeeId}</span></td><td>{record.department}</td><td>{record.schedule}</td><td className={record.timeIn === "—" ? "hr-reports-missing-value" : undefined}>{record.timeIn}</td><td className={record.timeOut === "—" ? "hr-reports-missing-value" : undefined}>{record.timeOut}</td><td><SourceBadge source={record.source} /></td><td><StatusBadge tone={record.statusTone}>{record.status}</StatusBadge></td><td><StatusBadge tone={record.validationTone}>{record.validationStatus}</StatusBadge></td><td><StatusBadge tone={record.hrVerificationStatus === "Verified" ? "success" : record.hrVerificationStatus === "Needs Correction" ? "warning" : "info"}>{record.hrVerificationStatus ?? "Pending Review"}</StatusBadge></td><td><StatusBadge tone={record.payrollReadiness === "Ready for Payroll" ? "success" : "muted"}>{record.payrollReadiness ?? "Not Ready"}</StatusBadge></td></tr>)}{records.length === 0 ? <EmptyReportRow columns={10} /> : null}</tbody>
      </table>
    </div>
  );
}
